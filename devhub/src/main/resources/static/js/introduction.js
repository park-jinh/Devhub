      ...
      $.ajax({
         ...
         success: function(allSkList2) {
            ...
            allSkList2.forEach(function(sk, index) {
                let checkboxId = "checkbox-item-" + (index + 11); // 고유한 ID 생성
                    let checkboxHtml = "<li><div class='flex items-center ps-2 rounded hover:bg-gray-100 dark:hover:bg-gray-600'>" + 
                        "<input id='" + checkboxId + "' type='checkbox' value='" + sk.skill_id + "' class='w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-700 dark:focus:ring-offset-gray-700 focus:ring-2 dark:bg-gray-600 dark:border-gray-500'>" + 
                        "<label for='" + checkboxId + "' class='w-full py-2 ms-2 text-sm font-medium text-gray-900 rounded dark:text-gray-300'>" + sk.skill_name + "</label></div></li>";
                    $("#formSkillList").append(checkboxHtml);
...
function filterSkills(value) {
   ...
   let skills = document.querySelectorAll('#formSkillList li');
   skills.forEach(function(skill) {
      let skillName = skill.textContent || skill.innerText;
...
function addSkillToUI(skillId) {
   ...
   $.ajax({
      ...
      success: function(skill) {
         ...
         let newItem = '<li>' + skill.skill_name + '</p><img src="' + skill.skill_img + '" /></li>';
...
function submitUserIntro() {
   let introData = document.getElementById("user_intro").value;
...
function submitGitHub() {
   let gitData = document.getElementById("github").value;
