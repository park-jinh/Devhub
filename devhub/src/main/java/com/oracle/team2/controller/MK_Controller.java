...
public class MK_Controller {
	...
	public String SelfIntroduction(User user, Model model, HttpServletRequest request) {
		...
		System.out.println("MK_Controller SelfIntroduction skill.size() introduction... " +introduction);
		model.addAttribute("datas", introduction);
		if(skill != null && skill.get(0) != null)
			model.addAttribute("skills", skill);
		System.out.println("MK_Controller SelfIntroduction skill.size() ... " + skill.size());
		System.out.println("MK_Controller SelfIntroduction skill --> " + skill);
...
//		프로젝트 지원 현황 리스트 게시판 
	...
	@GetMapping("MyScore")
	public String ProjectApplicationList(User user,Model model, HttpServletRequest request) {
		...
		if(myScore.size() == 0 || userScore.size() == 0)
			return "redirect:"+ request.getHeader("Referer");
